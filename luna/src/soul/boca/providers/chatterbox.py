...
class ChatterboxProvider(TTSProvider):
    ...
    def check_availability(self) -> bool:
        from src.soul.boca.engine_check import _check_chatterbox_internal

        result = _check_chatterbox_internal(self._boca)
        if result:
            self._venv_python = self._boca.venv_tts_python
            self._wrapper_path = self._boca.chatterbox_wrapper_path
            self._reference_audio = self._boca.chatterbox_reference_audio
    ...
    @perf_monitor("tts.provider.chatterbox.generate")
    def generate(self, text: str, params: TTSParams) -> str | None:
        ...
        try:
            ...
            logger.info(f"[Chatterbox] Audio gerado: {output_path}")
            return output_path
    ...
    def speak(self, text: str, params: TTSParams) -> bool:
        ...
        try:
            ...
            logger.info(f"[Chatterbox] Audio gerado em {duration:.2f}s")
            ...
            logger.error(f"Erro no Chatterbox speak: {e}")
            return False
        finally:
